    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // ******** START MY AND TUTORIAL CODE ********
          int prev_size = previous_path_x.size(); // From video tutorial -> use the size of the last path
          ...
          for(int i=0; i < sensor_fusion.size(); i++){    // check all "other" cars on the map
            ...
            //car is in my lane -> do something!
            float d = sensor_fusion[i][6];    // That car's d position in frenet coordinates 
            if( d < (2+4*lane+2) && d > (2+4*lane-2) ){ // car is infront of me
              ...
              // check s values are greater than these and s gap
              if((check_car_s > car_s) && ((check_car_s-car_s) < 30) ){ // the car must be in front of us, and the gap is around 30 meters

                
                //ref_vel = 29.5;  // mph --> example from tutorial
                too_close = true;

                /*
                // Example to change lane from the tutorial
                if(lane > 0){
                  lane = 0;
                }*/
